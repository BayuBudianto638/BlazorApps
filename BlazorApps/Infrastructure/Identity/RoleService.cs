...
namespace Infrastructure.Identity
{
    internal class RoleService : IRoleService
    {
        ...
        public async Task<string> CreateDepartmentRolesAsync(List<CreateDepartmentRole> request, string departmentId, CancellationToken cancellationToken)
        {
            var departmentRoles = await _db.DepartmentRoles.Where(x => x.DepartmentId == Guid.Parse(departmentId)).ToListAsync();
            Console.WriteLine(request.Count);
            if (departmentRoles.Any())
            {
                foreach (var item in departmentRoles)
                {
                    _db.DepartmentRoles.Remove(item);
                }
                await _db.SaveChangesAsync(cancellationToken);
                foreach (var item in request)
                {
                    DepartmentRole departmentRole = new DepartmentRole(item.DepartmentId, item.RoleId);
                    await _db.DepartmentRoles.AddAsync(departmentRole);
                }
                await _db.SaveChangesAsync(cancellationToken);
            }
            else
        ...
        public async Task<List<DepartmentRoleDto>> GetDepartmentRolesListAsync(string id, CancellationToken cancellationToken) =>
            (await _db.DepartmentRoles.Where(x => x.DepartmentId == Guid.Parse(id)).ToListAsync(cancellationToken)).Adapt<List<DepartmentRoleDto>>();
