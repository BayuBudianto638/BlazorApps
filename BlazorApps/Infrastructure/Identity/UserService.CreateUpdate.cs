...
namespace Infrastructure.Identity
{
    internal partial class UserService
    {
        ...
        public async Task<string> CreateAsync(CreateUserRequest request, string origin)
        {
            ...
            var departmentRoles = await _db.DepartmentRoles.Where(x => x.DepartmentId == request.DepartmentId).ToListAsync();
            foreach (var departmentRolesId in departmentRoles)
            {
                await _userManager.AddToRoleAsync(user, (await _roleManager.FindByIdAsync(departmentRolesId.RoleId.ToString())).Name);
            }
        ...
        public async Task<string> ChangeDepartmentAsync(DepartmentChange request, string userId)
        {
            var user = await _userManager.FindByIdAsync(userId);
            ...
            if (!result.Succeeded)
            {
                throw new InternalServerException(_localizer["Update Department failed"], result.GetErrors(_localizer));
            }

            return string.Join(Environment.NewLine, result.GetErrors);
        }
    }
}